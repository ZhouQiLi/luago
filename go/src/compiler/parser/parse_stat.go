...
func _finishForInStat(lexer *Lexer, varName string) *ForInStat {
	nameList := _finishNameList(lexer, varName)
...
func _finishNameList(lexer *Lexer, key string) []string {
	names := []string{key}
...
func parseAssignStat(lexer *Lexer, exp Exp) *AssignStat {
	varList := _finishVarList(lexer, exp)
	lexer.NextTokenOfKind(TOKEN_OP_ASSIGN)
	expList := parseExpList(lexer)
	lastLast := lexer.Line()
	return &AssignStat{lastLast, varList, expList}
}

func _finishVarList(lexer *Lexer, exp0 Exp) []Exp {
	vars := []Exp{_checkVar(lexer, exp0)}
	for lexer.LookAhead() == TOKEN_SEP_COMMA {
		lexer.NextToken()
...
func _parseFuncName(lexer *Lexer) (exp Exp, hasColon bool) {
	...
	for lexer.LookAhead() == TOKEN_SEP_COLON {
		lexer.NextToken()
		line, name := lexer.NextIdentifier()
		idx := &StringExp{line, name}
		exp = &TableAccessExp{line, exp, idx}
