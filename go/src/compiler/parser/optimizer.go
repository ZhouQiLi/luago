...
func optimizeArithBinaryOp(exp *BinopExp) Exp {
	...
	if x, ok := castToFloat(exp.Exp1); ok {
		if y, ok := castToFloat(exp.Exp2); ok {
			...
			case TOKEN_OP_IDIV:
				if y != 0 {
					return &FloatExp{exp.Line, number.FFloorDiv(x, y)}
				}
			case TOKEN_OP_MOD:
				if y != 0 {
					return &FloatExp{exp.Line, number.FMod(x, y)}
				}
			case TOKEN_OP_POW:
				return &FloatExp{exp.Line, math.Pow(x, y)}
			}
		}
	}
